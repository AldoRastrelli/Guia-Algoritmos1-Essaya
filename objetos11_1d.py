class Intervalo:
    ...
    def union(self,otro):
        """recibe otro intervalo. Si los intervalos no son adyacentes ni intersectan, 
        debe lanzar una excepción. En caso contrario, devuelve un nuevo intervalo resultante de la union
        entre ambos"""
        
        if max(self.desde,otro.desde) <= min(self.hasta,otro.hasta):
            return self.desde,otro.hasta
        raise Exception('Los intervalos no son adyacentes ni se intersecan')
