class Vector:
    ...
    def __mul__(self,multiplicador):
        """recibe un numero y devuelve un nuevo vector, con los elementos multiplicados
        por ese numero"""

        if not isinstance(multiplicador,(int,float,complex)):
            print('El multiplicador ingresado no es un número')
            return None
        
        return Vector([self.items[i]*multiplicador for i in range(len(self.items))])